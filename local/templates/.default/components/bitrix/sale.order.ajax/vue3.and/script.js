(function (root, factory) {
    ...
    gsvBasketSoa.prototype.setup = function () {
        ...
        watch(data.model.PROPERTIES, (...args) => this.onInputFilledChanged(...args)/*, { immediate: true }*/);
        onMounted(() => $(() => setTimeout(() => this.onInputFilledChanged())));
        ...
        watch(() => this.$isSelfDelivery.value, (...args) => this.onSelfDeliveryChanged(...args));
        ...
        return {
            ...
            fastBuy: (...args) => this.fastBuy(...args),
...
//item.AVAILABLE_QUANTITY *= 5;
    ...
    gsvBasketSoa.prototype.onInputFilledChanged = function () {
        $("input[type=text], input[type=email], input[type=tel], input[type=password], textarea").trigger("change");
        $("textarea").trigger('keyup');
    }

    gsvBasketSoa.prototype.onSelfDeliveryChanged = async function (isSelfDelivery, wasSelfDelivery) {
        if(wasSelfDelivery && !isSelfDelivery) {
            this.data.model.PROPERTIES.ADDRESS = '';
        } else if(!wasSelfDelivery && isSelfDelivery) {
            this.data.model.PROPERTIES.ADDRESS = '-';
        }
        await nextTick();
        this.onInputFilledChanged();
    }
    ...
    gsvBasketSoa.prototype.parseProperties = function (order) {
        ...
        for (let i in order.ORDER_PROP.properties) {
            ...
            if(property.IS_ADDRESS === 'Y' || property.IS_ADDRESS === true) {
                properties['SDEK__' + code + "__ID"] = property.ID;
            }
        }
    ...
    gsvBasketSoa.prototype.fillProperties = function (confirm) {
        ...
        for (let i in order.ORDER_PROP.properties) {
            ...
            let code = property.CODE;
            code = code.toUpperCase();
            result['ORDER_PROP_' + property.ID] = model.PROPERTIES[code];
    ...
    gsvBasketSoa.prototype.addQty = async function(item, val) {
        item.edited = false;

        let body = {};

        body[`basket[QUANTITY_${item.ID}]`] = item.QUANTITY + val;
        //To prevent blinking...
        item.QUANTITY = body[`basket[QUANTITY_${item.ID}]`];
    ...
    gsvBasketSoa.prototype.removeQty = async function(item, btn) {
        let doIt = await fancyAnswer(btn);

        if(doIt) {
            await this.removeQtyImpl(item)
        }
    }

    gsvBasketSoa.prototype.fastBuy = async function($event) {
        this.preventDefault($event);
        $("body").addClass("side-modal-opened");
        $(".side-modal.fastbuy-modal").addClass("show");
        $(".side-modal-overlay").addClass("show");
    }

    gsvBasketSoa.prototype.initFastBuy = function(variant, props) {
        let self = this;
        this.isBuying = false;
        ...
        $('#fastbuy-modal button[data-buy-button]').on('click', async function () {
            let $this = $(this);
            if($this.hasClass('btn-disabled') || self.isBuying) {
                return;
            }
            self.isBuying = true;
            let result = await self.buyNow();
            self.isBuying = false;
            if(result && result.order_id) {
                $('#fastbuy-modal .fastbuy-success-message').removeClass('d-none');
                //TODO: What to do now!?
                let url = new URL(window.location.href);
                url.search = "?ORDER_ID=" + result.order_id;
    ...
    gsvBasketSoa.prototype.showBuyBtn = function ($form) {
    ...
    gsvBasketSoa.prototype.buyNow = async function () {
        let req = {};
    ...
    gsvBasketSoa.prototype.removeQtyImpl = async function(item) {
        let body = {};
    ...
    gsvBasketSoa.prototype.onConfirmOrder = async function ($event) {
        ...
        if(result.ERROR) {
            let errors = [];
            for(let type in result.ERROR) {
                let errorOfType = result.ERROR[type];
        ...
        await new Promise(function (resolv) {
            setTimeout(resolv, 5000);
        })
        if(result.REDIRECT_URL) {
            window.location.href = result.REDIRECT_URL;
        } else if(result.success == 'Y' && result.redirect) {
    ...
    gsvBasketSoa.prototype.confirmOrder = async function () {
        ...
        //let result = await this.processOrder(true);
    ...
    gsvBasketSoa.prototype.bitrix_sessid = function () {
        return this._opt.AJAX.sessid;
    }
