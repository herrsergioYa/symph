<?if (!defined("B_PROLOG_INCLUDED") || B_PROLOG_INCLUDED !== true) die();
\Bitrix\Main\Localization\Loc::loadLanguageFile(__FILE__);
$_SESSION['ORDER_ACCOUNT_NUMBER'] = $arResult['ACCOUNT_NUMBER'];

$APPLICATION->SetPageProperty('page-type', "");
$APPLICATION->SetPageProperty('title', \data::get('CHECKOUT_TITLE'));
$APPLICATION->SetTitle(\data::get('CHECKOUT_TITLE'));
?>
<div class="container-fluid d-flex align-items-center justify-content-center h-100">
    <div>
        <h1 class="tac">Cпасибо за заказ!</h1>
        <div class="mw-400 my-5 pt-3 pb-3">
            Номер заказа: <?= $arResult['ACCOUNT_NUMBER'] ?><br>
            Инорфмация о&nbsp;вашем заказе отправлена на&nbsp;электронную почту, а&nbsp;также доступна в&nbsp;<a class="wsn u-line" href="<?= SITE_DIR?>personal/orders/">личном кабинете</a>.
        </div>
        <div class="tac">
            <a class="btn btn-blank py-0" href="<?= $arParams['PATH_TO_CATALOG']?>"><?= \data::get('BACK_TO_CATALOG') ?></a>
        </div>
    </div>
</div>