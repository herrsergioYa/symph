...
//	$arFilter["SHOW_HISTORY"] = $WF_SHOW_HISTORY;
    ...
    if($obElement = $rsElement->fetchObject())
	{
		...
		$arResult['RAW'] = $obElement->collectValues();

        if($arParams['DETAIL_URL'])
        {
            $arResult['DETAIL_PAGE_URL'] = $arParams['DETAIL_URL'];
            $arResult['DETAIL_PAGE_URL'] = str_replace('#ORM#', $dataClass::getEntity()->GetName(), $arResult['DETAIL_PAGE_URL']);
            $arResult['DETAIL_PAGE_URL'] = str_replace('#ELEMENT_ID#', $arResult['RAW']['ID'], $arResult['DETAIL_PAGE_URL'] );
            if($arParams['CODE_FIELD'])
            {
                $arResult['DETAIL_PAGE_URL'] = str_replace('#ELEMENT_CODE#', $arResult['RAW'][$arParams['CODE_FIELD']], $arResult['DETAIL_PAGE_URL']);
            }
        }

        if($arParams['LIST_URL'])
        {
            $arResult['LIST_PAGE_URL'] = $arParams['LIST_URL'];
            $arResult['LIST_PAGE_URL'] = str_replace('#ORM#', $dataClass::getEntity()->GetName(), $arResult['LIST_PAGE_URL']);
            $arResult['LIST_PAGE_URL'] = str_replace('#ELEMENT_ID#', $arResult['RAW']['ID'], $arResult['LIST_PAGE_URL'] );
            if($arParams['CODE_FIELD'])
            {
                $arResult['LIST_PAGE_URL'] = str_replace('#ELEMENT_CODE#', $arResult['RAW'][$arParams['CODE_FIELD']], $arResult['LIST_PAGE_URL']);
            }
...
//		);
		...
		foreach($arParams["FIELD_CODE"] as $code)
			if(array_key_exists($code, $arResult['RAW']))
				$arResult["FIELDS"][$code] = $arResult['RAW'][$code];
