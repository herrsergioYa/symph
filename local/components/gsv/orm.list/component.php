...
if(!CModule::IncludeModule("iblock")/* || !\Bitrix\Main\Loader::includeModule('highloadblock')*/)
    return;
...
$arParams["DATA_CLASS"] = trim($arParams["DATA_CLASS"]);
...
//	if($arNavigation["PAGEN"]==0 && $arParams["PAGER_DESC_NUMBERING_CACHE_TIME"]>0)
    ...
    if(!empty($arNavigation["PAGEN"]))
    {
        $arNewNavParams['offset'] = $arNewNavParams['limit'] * ($arNavigation["PAGEN"] - 1);
    }
}
else
{
...
if($this->startResultCache(false, array(($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()), $bUSER_HAVE_ACCESS, $arNavigation, $arrFilter, $pagerParameters)))
{
    ...
    {
        $this->abortResultCache();
        ShowError(GetMessage("HLBL_MODULE_NOT_INSTALLED"));
        return;
    }
//	if(is_numeric($arParams["HLBL_ID"]))
//	{
...
//	}
    ...
    //$entity = \Bitrix\Highloadblock\HighloadBlockTable::compileEntity($arResult);
	//$dataClass = $entity->getDataClass();
    $dataClass = $arParams['DATA_CLASS'];
	if (!class_exists($dataClass, true) || !is_subclass_of($dataClass, '\Bitrix\Main\ORM\Data\DataManager'))
	{
		$this->abortResultCache();
		Iblock\Component\Tools::process404(
			trim($arParams["MESSAGE_404"]) ?: GetMessage("T_NEWS_NEWS_NA")
...
//	}
	...
	$rsElement = $dataClass::getList($arList);
	//$rsElement->SetUrlTemplates($arParams["DETAIL_URL"], "", $arParams["IBLOCK_URL"]);
	while($obElement = $rsElement->fetchObject())
	{
		$arItem = [
            'RAW' => $arItem = $obElement->collectValues(),
            'ORM_OBJECT' => $obElement
        ];

		if($arParams['DETAIL_URL'])
        {
            $arItem['DETAIL_PAGE_URL'] = $arParams['DETAIL_URL'];
            $arItem['DETAIL_PAGE_URL'] = str_replace('#ORM#', $dataClass::getEntity()->GetName(), $arItem['DETAIL_PAGE_URL']);
            $arItem['DETAIL_PAGE_URL'] = str_replace('#ELEMENT_ID#', $arItem['RAW']['ID'], $arItem['DETAIL_PAGE_URL']);
            if($arParams['CODE_FIELD'])
            {
                $arItem['DETAIL_PAGE_URL'] = str_replace('#ELEMENT_CODE#', $arItem['RAW'][$arParams['CODE_FIELD']], $arItem['DETAIL_PAGE_URL']);
            }
        }

        if($arParams['LIST_URL'])
        {
            $arItem['LIST_PAGE_URL'] = $arParams['LIST_URL'];
            $arItem['LIST_PAGE_URL'] = str_replace('#ORM#', $dataClass::getEntity()->GetName(), $arItem['LIST_PAGE_URL']);
            $arItem['LIST_PAGE_URL'] = str_replace('#ELEMENT_ID#', $arItem['RAW']['ID'], $arItem['LIST_PAGE_URL'] );
            if($arParams['CODE_FIELD'])
            {
                $arItem['LIST_PAGE_URL'] = str_replace('#ELEMENT_CODE#', $arItem['RAW'][$arParams['CODE_FIELD']], $arItem['LIST_PAGE_URL']);
            }
        }
...
//		);
		...
		foreach($arParams["FIELD_CODE"] as $code)
			if(array_key_exists($code, $arItem))
				$arItem["FIELDS"][$code] = $arItem['RAW'][$code];
	}
	unset($arItem);
...
//		}
	...
	$arResult['DATA_CLASS'] = $arParams['DATA_CLASS'];

    if(false !== $arNavigation)
    {
        unset($arList['limit'], $arList['offset']);
        $rsElement = $dataClass::getList($arList);
        $arResult["NAV_RESULT"] = new CDBResult($rsElement);
        $arResult["NAV_RESULT"]->InitNavStartVars($arNavParams['nPageSize'], $arNavParams['bShowAll'], $arNavigation["PAGEN"]);
        $dbresult = &$arResult["NAV_RESULT"];

        $dbresult->NavRecordCount = $arResult["NAV_RESULT"]->SelectedRowsCount();
       // $dbresult->NavPageSize;
        $dbresult->NavPageCount = ceil($dbresult->NavRecordCount / $dbresult->NavPageSize);
        $dbresult->NavPageNomer = $arNavigation["PAGEN"];
        $dbresult->bShowAll = $arNavParams['bShowAll'];
        $dbresult->NavShowAll = $arNavParams['bShowAll'];
       // $dbresult->NavNum;
     //   $dbresult->bDescPageNumbering;
    //    $dbresult->add_anchor;
      //  $dbresult->nPageWindow;

      //  $arResult["NAV_RESULT"]->NavRecordCount = 70;
        $arResult["NAV_RESULT"]->nSelectedCount = count($arResult['ITEMS']);
