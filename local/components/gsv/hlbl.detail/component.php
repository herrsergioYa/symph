...
if($arParams["SHOW_WORKFLOW"] || $this->startResultCache(false, array(($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()),$bUSER_HAVE_ACCESS, $arNavigation, $arrFilter, $pagerParameters)))
{
...
//		);
    ...
    elseif($arParams['CODE_FIELD'])
    {
        $arFilter[$arParams['CODE_FIELD']] = $arParams["ELEMENT_CODE"];
    }
...
//	$arFilter["SHOW_HISTORY"] = $WF_SHOW_HISTORY;
    ...
    $rsElement = $dataClass::getList([
        'filter' => array_merge($arFilter, $arrFilter),
        'select' => $arSelect,
    ]);
	//$rsElement->SetUrlTemplates($arParams["DETAIL_URL"], "", $arParams["IBLOCK_URL"]);
	//if($obElement = $rsElement->GetNextElement())
    if($obElement = $rsElement->fetchObject())
	{
		$arResult/*['ITEM']*/ = $obElement->collectValues();
        $arResult['ORM_OBJECT'] = $obElement;

        if($arParams['DETAIL_URL'])
        {
            $arResult/*['ITEM']*/['DETAIL_PAGE_URL'] = $arParams['DETAIL_URL'];
            $arResult/*['ITEM']*/['DETAIL_PAGE_URL'] = str_replace('#HLBL_ID#', $arParams['HLBL_ID'], $arResult/*['ITEM']*/['DETAIL_PAGE_URL']);
            $arResult/*['ITEM']*/['DETAIL_PAGE_URL'] = str_replace('#ELEMENT_ID#', $arResult['ITEM']['ID'], $arResult/*['ITEM']*/['DETAIL_PAGE_URL'] );
            if($arParams['CODE_FIELD'])
            {
                $arResult/*['ITEM']*/['DETAIL_PAGE_URL'] = str_replace('#ELEMENT_CODE#', $arResult/*['ITEM']*/[$arParams['CODE_FIELD']], $arResult/*['ITEM']*/['DETAIL_PAGE_URL']);
            }
        }

        if($arParams['LIST_URL'])
        {
            $arResult/*['ITEM']*/['LIST_PAGE_URL'] = $arParams['LIST_URL'];
            $arResult/*['ITEM']*/['LIST_PAGE_URL'] = str_replace('#HLBL_ID#', $arParams['HLBL_ID'], $arResult/*['ITEM']*/['LIST_PAGE_URL']);
            $arResult/*['ITEM']*/['LIST_PAGE_URL'] = str_replace('#ELEMENT_ID#', $arResult/*['ITEM']*/['ID'], $arResult/*['ITEM']*/['LIST_PAGE_URL'] );
            if($arParams['CODE_FIELD'])
            {
                $arResult/*['ITEM']*/['LIST_PAGE_URL'] = str_replace('#ELEMENT_CODE#', $arResult/*['ITEM']*/[$arParams['CODE_FIELD']], $arResult/*['ITEM']*/['LIST_PAGE_URL']);
            }
        }
...
//		);
		...
		foreach($arParams["FIELD_CODE"] as $code)
			if(array_key_exists($code, $arResult['ITEM']))
				$arResult["FIELDS"][$code] = $arResult['ITEM'][$code];
...
//		}
	...
	else
	{
        ...
        $arResult['ID'] = 0; //HACK: It shouldn't be set at all. It is a mistake. Isn't it?
        $arResult['HLBL_ID'] = $arParams['HLBL_ID'];
        $this->setResultCacheKeys(['HLBL_ID']);
        $this->includeComponentTemplate();
	}
}

return intval($arResult['ITEM']['ID']);
