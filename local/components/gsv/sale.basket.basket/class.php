...
class CGsvBitrixBasketComponent extends CBitrixComponent
{
	...
	{
        if(is_array($offerProps))
        {
            $res = $this->changeProductOfferWithoutSave($id, self::SEARCH_OFFER_BY_PROPERTIES, $offerProps, false);
        }
        else if(is_numeric($offerProps))
        {
            $res = $this->changeProductOfferWithoutSave($id, self::SEARCH_OFFER_BY_ID, ['ID' => $offerProps], false);
        }
        else
        {
            $this->addErrors(['400 Incorrect request'], $id);
            return;
        }
		if (!$res->isSuccess())
		{
	...
	protected function changeProductOfferWithoutSave($basketId, $searchType, $searchData, $useMerge = true)
	{
		...
		else
		{
            ...
            if($newProduct) {
                $parent = CCatalogSku::getProductList($newProduct['ID'], 0);
                $parent = $parent[$newProduct['ID']];
            }
		}
		...
		if ($result === null)
		{
			...
			foreach ($codeList as $code)
			{
				...
				if (empty($currentOffers))
				{
					...
					{
						if (isset($existValues[$valueId]) && $valueId == $propertyValues[$code])
						{
	...
	protected function selectOfferById($iblockId, $productId, $currentOfferId, $newOfferId, $propertyCodes)
	{
        $arProduct = \CIBlockElement::GetList(
            ...
            false,
            ['nTopCount' => 1],
            ['ID', 'IBLOCK_ID', 'NAME', 'XML_ID', 'PROPERTY_CML2_LINK']
        )->Fetch();
		return $arProduct ?: null;
	}
